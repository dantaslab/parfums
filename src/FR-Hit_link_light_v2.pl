...
foreach my $l (keys(%links)){
  foreach my $t (keys(%{$links{$l}})){
    if ($links{$l}{$t} >= $linkThreshold){
      ...
      system("perl /opt/apps/labs/gdlab/software/parfums/1.0/src/FR-Hit_cleanLink.pl $linkMap_file $toGet_Seq_file");
      ...
      system("perl /opt/apps/labs/gdlab/software/parfums/1.0/src/GetSequences.pl $toGet_Seq_file $dirname/$prefix.fasta.clean >> $toMap_file");
      ...
      system("perl /opt/apps/labs/gdlab/software/parfums/1.0/src/FR-Hit_cleanChimera.pl $linkMap_file $linkOut_file.contigs > $noChimera_file");
