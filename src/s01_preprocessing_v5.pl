...
##                     sequencing data(fastq files) and bin the reads into different files
##                     based on barcodes.
##
...
my $bc_filename = basename($barcode_file);
$logger -> info("Reading barcode $bc_filename");
	...
	while(defined($fwline = <$fwdIN>) && defined($reline = <$revIN>)) {
		...
		$total++;
			...
			if ((exists $barcodes{$fwdBar}) && ($fwdBar eq $revBar)) {
				...
				&write_record($fileT[0], 2);
				&write_record($fileT[1], 3);
			}
			else {
				if ($unmatch) {
...
sub create_output_files {
	...
	foreach my $fileName (keys %counts) {
		open (my $fileOut_FW, "> $outputDir/PreprocessedFiles/$fileName\_FW.fq") or logger-> logdie ("Error in opening a directory:$!");
		open (my $fileOut_RC, "> $outputDir/PreprocessedFiles/$fileName\_RV.fq") or logger-> logdie ("Error in opening a directory:$!");
		$trackFiles{$fileName} = [ $fileOut_FW,$fileOut_RC ];
	}
}
