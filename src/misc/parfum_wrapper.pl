...
foreach my $proc (@procStep) {
    if ($proc == 1) {
        my $processFlag = 0;
        my $cmd = "perl $dirname/src/preprocessing_v4.pl --bcfile $config{'BC_file'} --fwd_file $config{'FW_file'} --rev_file $config{'RC_file'} --$config{'barcode_pos'} --dir $config{'DIR'}";
        if ($unmatch_stat) {
            $cmd = $cmd . " --unmatch";
        }
	...
	if ($exit) {
        ...
        if ($processFlag) {
            my $cmd1 = "perl $dirname/src/fastq_to_fasta_v1.pl --dir $config{'DIR'} --bcfile $config{'BC_file'}";
	    $logger ->info("$cmd1");
        ...
        my $processFlag = 0;
        my $cmd = "perl $dirname/src/make_adapter_run_CM.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --adapA $config{'adapA'} --adapB $config{'adapB'} --endVec1 $config{'endVec1'} --endVec1R $config{'endVec1R'} --endVec2 $config{'endVec2'} --endVec2R $config{'endVec2R'}";
        $logger ->info("Step 2: $cmd");
	my ($stdout,$stderr,$exit) = capture { system("$cmd"); };
        #if ($exit) {
        #    &error_Notify($proc,$stderr);
        #} else {
	...
	$logger -> info("STEP 2: ADAPTER TRIMMING COMPLETED");
        ...
        my $processFlag = 1;
        my $cmd = "perl $dirname/src/clean_Phix.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --phix $config{'phixDIR'}";
        ...
        if ($processFlag) {
            ...
            my $cmd = "perl $dirname/src/remove_Vectors.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --vecfile $config{'vecfile'}";
            my ($stdout1,$stderr1,$exit1) = capture { system("$cmd"); };
	...
	$logger -> info("STEP 3: PhiX and VECTOR SEQUENCES REMOVED");
        ...
        my $cmd = "perl $dirname/src/velvet_assembly_v1.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --unchecked $IDs_Not_Passed";
        $logger -> debug("$cmd");
	my ($stdout,$stderr,$exit) = capture {
            system("$cmd");
        };
        ...
        my $cmd = "perl $dirname/src/phrap_assembly_v2.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --unchecked $IDs_Not_Passed";
        #print "$cmd\n";
	my ($stdout,$stderr,$exit) = capture {
            system("$cmd");
        };
	...
	if ($config{'resANNOTATE'}) {
		$cmd = "perl $dirname/src/annotate_v1.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --unchecked $IDs_Not_Passed --resannotate $config{'resANNOTATE'}";
	}
	else { 
		$cmd = "perl $dirname/src/annotate_v1.pl --bcfile $config{'BC_file'} --dir $config{'DIR'} --unchecked $IDs_Not_Passed";
        }
	#print "$cmd\n";
	my ($stdout,$stderr,$exit) = capture {
            system("$cmd");
...
sub check_output_files {
    ...
    while(my $line = <$in>) {
        ...
        if (!-e $outfile || -z $outfile) {
            $logger ->logdie("$seqID[1].$suffix is empty. Halting its processing here");
            if ($IDs_Not_Passed) {
