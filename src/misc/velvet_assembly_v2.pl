...
my ($sackedIDs, $bc_file, $inputDir, $paramVelveth1, $paramVelvetg1, $runType, $jobFile);
...
foreach my $seqID (keys %inputSeqFile) {
	my $dirname = "$inputDir/$seqID";
	#my $dirname = dirname($inputSeqFile{$seqID});	
	my $file_v1 = "$dirname/$seqID\_velvet1_contigs.fasta";
	my $file_v2 = "$dirname/$seqID\_velvet2_contigs.fasta";
	my $file_v3 = "$dirname/$seqID\_velvet3_contigs.fasta";
	if (-z $file_v1) && (-z $file_v2) && (-z $file_v3) {
		push(@uncheckIDs, $seqID);
	}
}		
