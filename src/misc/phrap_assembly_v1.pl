...
my($bc_file, $inputDir, $paramPhrap1, $paramPhrap2, $jobFile, $sackedIDs, $resAnnotate);
...
$jobFile = &generate_script("annotate", \%contigFile, "annotation.txt");
...
sub get_PhrapCmd {
    ...
    foreach my $sampleId ( keys %seqFile){
	    ...
	    $contigFile{$sampleId} = $OUTFILE;
            ...
            $logger -> logdie("File $file doesnt exists: $!");
        }
    }
    return \@cmds;
}
...
sub get_LinkContigsCmd {
    ...
    foreach my $sampleId ( keys %cdhitFile){
        ...
        if(-e $file){
            ...
            my $CDHITFILE = $file;
            my $OUTFILE = "$dirname/$sampleId\.$suffix";
            my $tmp_cmd = "perl $Bin/FR-Hit_link_light.pl $MAPFILE $CDHITFILE $sampleId $OUTFILE";
    ...
    foreach my $sampleId ( keys %splitFile){
        if(exists $splitFile{$sampleId}){
            ...
            foreach my $file (@files){
                ...
                my $tmp_cmd = "blastx -outfmt 6 -evalue 1E-4 -seg no -db $maindir/datasets/COG_with_len.fa -query $INFILE > $OUTFILE";
...
sub get_parseBlastCOGCmd{
    ...
    foreach my $sampleId ( keys %seqFile){
        ...
        if( -e $file){
            ...
            my $tmp_cmd = "$Bin/Preparse_Cog_Blast1_Gautam.pl $maindir/datasets/COG.mappings.v8.2.txt $CDHIT_FILE $BLASTCOG_FILE > $PREPARSE_FILE ;";
...
sub get_BlastForStitchCmd{
    ...
    foreach my $sampleId ( keys %splitFile){
		...
		$logger -> debug("Blast_to_Stitch: inFile:outFile\t$INFILE\t$OUTFILE");
                ...
                $tmp_cmd .= " blastn -query $INFILE -db $CDHIT_FILE -outfmt 6 -evalue 1e-7 -dust no -out $OUTFILE";
                push(@cmds, $tmp_cmd);
            }
        }
    }
